...
func NewServer(host string, port int, mode string, corsc *cors.Config{{ if .prometheus}}, promdisabled bool, promprefix string{{ end }}) GinServer {
	s := GinServer{}
	s.port = port
	s.host = host
	setMode(mode)
